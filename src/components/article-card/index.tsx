...
export function ArticleCard({ article, badge }: { article: ArticleRecord; badge: string }) {
  return (
    <Link
      href={article.url}
      target="_blank"
    ...
    >
      ...
      {article.imageUrl ? (
        <div className="relative aspect-[16/9] overflow-hidden rounded-2xl">
          <Image src={article.imageUrl} alt={article.title} fill className="object-cover transition duration-500 group-hover:scale-[1.03]" />
        </div>
      ) : null}
