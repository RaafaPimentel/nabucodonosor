...
export function scoreArticle(article: NormalizedArticle, category: NewsCategory): ScoreDetails {
  ...
  const categoryAdjustment =
    category.id === "crypto-digital-assets"
      ? haystack.includes("price") && !haystack.includes("regulation") && !haystack.includes("stablecoin")
        ? -0.08
        : 0
      : category.id === "product"
        ? haystack.includes("pricing") || haystack.includes("monetization") || haystack.includes("subscription")
          ? 0.04
          : 0
        : 0;
  const score = Number(
    Math.max(
      0,
      recencyScore * 0.35 + keywordScore * 0.3 + credibilityScore * 0.2 + imageScore * 0.08 + specificityScore * 0.07 + categoryAdjustment
    ).toFixed(4)
  );
